...
echo "Exec=konsole -e script -f -c 'set -x; yum install xterm -y; xterm -e \"set -x; tail -F /root/Desktop/v3.5\\\\ Update\\\\ Combo/logs/Stage${1}.txt\"& cd /root/Desktop/v3.5\ Update\ Combo; ./scripts/${1}.sh' /root/Desktop/v3.5\ Update\ Combo/logs/Stage${1}.txt" >> '/root/Desktop/v3.5 Update Combo/scripts/next.desktop' || return 1
